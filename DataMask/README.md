...
Due to restrictions in MySQL it's impossible to implement `gen_dictionary`, `gen_dictionary_drop` and `gen_dictionary_load` as functions, because only procedures could use prepared statements. The procedure `gen_dictionary_load` cannot load it's dictionary into the temporary table, because of restrictions on allowed SQL in prepared statements (see [SQL Syntax Permitted in Prepared Statements](https://dev.mysql.com/doc/refman/8.4/en/sql-prepared-statements.html#prepared-statements-permitted)). Functions `mask_inner`, `mask_outer` and `gen_rnd_pan` do not have optional argument, because this is still impossible in MySQL. The implementation could be completed if eventually MySQL removes these restrictions with future releases. Fingers crossed :-)
